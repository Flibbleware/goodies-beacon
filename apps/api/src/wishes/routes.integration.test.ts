...
describe.skipIf(!databaseUrl)('the wish list routes', () => {
  ...
  it('adds wishes and lists them newest first', async () => {
    await add({ label: 'Jurassic Park', category: 'vhs', searchUrl: SEARCH });
    await add({ label: 'Tamagotchi', category: 'toy' });

    const res = await app.request('/api/wishes', { headers: { cookie } });
    const { wishes } = (await res.json()) as { wishes: WishBody[] };

    expect(wishes.map((wish) => [wish.label, wish.category, wish.searchUrl])).toEqual([
      ['Tamagotchi', 'toy', null],
  ...
  it('stores every category', async () => {
    for (const category of ['game', 'dvd', 'vhs', 'toy', 'figurine', 'book', 'other']) {
      expect((await add({ label: category, category })).category).toBe(category);
    }
  });
  ...
  it('edits a wish in place, link included', async () => {
    const wish = await add({ label: 'Jurasic Park', category: 'dvd', searchUrl: SEARCH });
  ...
  it('removes a wish', async () => {
    const wish = await add({ label: 'x', category: 'other' });
  ...
  it('answers a missing or malformed id as not found rather than a 500', async () => {
    const body = { label: 'x', category: 'game' };
  ...
  it('promotes a wish once, however many times it is asked', async () => {
    const wish = await add({ label: 'x', category: 'game' });
