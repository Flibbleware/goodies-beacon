...
Version 1.41 — 25 September 2026. Written from the agreed requirements; this is the reference for the development plan that follows.
...
Reference and grading example images are not swept by age: they belong to a spec version or a scale and live as long as it does. What the same job does collect, from v1.35, is **orphaned media** — a file the spec editor stored whose entry was never saved into a version, because the upload happens on the click and the document that points at it only on save. A file that no spec version, grading scale or candidate references, and that is more than a day old, is deleted; the day is so that an editor left open over lunch does not lose its images.
