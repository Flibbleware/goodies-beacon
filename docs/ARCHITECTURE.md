...
Version 1.42 — 26 September 2026. Written from the agreed requirements; this is the reference for the development plan that follows.
...
*Wanted items* — a grid of cards, each headed by the item's display image or, without one, its category's icon on its tint, with the category named in a small badge over the corner of the picture; then the title, the status and mode as pills, the matched and uncertain counts in their verdict colours, and the last poll, whose date sits behind a clock (as it does on the item page); filterable by category, as the wish list is (cards from v1.42, P1-25, so the page reads as a shelf rather than as the wish list's rows). Item page: the candidate counts in their verdict colours, then sections that each fold away, all but the first starting folded — details (the summary and how sellers list it), settings, criteria, labelled reference images, and the search-plan table with per-query stats — with a pencil on each part of the spec opening an editor for that section alone, in a modal, whose save is a new version (v1.41, P1-24). Details carries the title, category and status too, a JSON button edits the whole document in a modal, and a clock beside it opens the version history in another; the full editor page is kept only for creating an item. The Reference Images section also holds the display image, chosen from the reference images or uploaded, and saved without a version. The version history gains diffs (Phase 3). The candidate list is filtered by day, verdict and origin — opening on today's matches, with each other verdict one filter away and no view of every verdict at once (v1.40). Today is the owner's day, dated as the dashboard's *Today* tiles date it — by the newest verdict, or by when a still-queued candidate was found — so a tile and the list it opens agree; *All* is one filter away, and an item page's all-time counts open it (v1.42). "Amend" opens the chat, and "Scan current listings" runs a backfill.
