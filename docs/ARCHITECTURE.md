...
Version 1.43 — 26 September 2026. Written from the agreed requirements; this is the reference for the development plan that follows.
...
**WantedItem** — `id, title, status (draft|active|paused|found|archived), categoryId?, displayImageId?, notificationMode (realtime|digest), pollEvery (ISO 8601 duration, nullable → global default), gradingScaleId?, minimumGrade?, currentSpecVersionId, createdAt`. `categoryId` (v1.37, P1-20; a reference from v1.39, P1-22) points at one of the owner's Categories, shared with the wish list so the two lists sort a collection the same way; it lives on the item beside the title rather than in the spec, because nothing searches or judges by it, and null is uncategorised. `displayImageId` (v1.42, P1-25) is the picture on the item's card in the list — one of the spec's reference images, or an upload used for nothing else — and references a media row with `on delete set null`. It is on the item for the reason the category is, and one more: the pipeline reads the spec and nothing else, so a photo chosen to look good on a shelf has no route into a review prompt however it was chosen. A reference image that is also the display image is still sent, because it is a reference image.

**The title, status, category and display image are the item's own fields, and changing them writes no version** (v1.42, P1-25). None of them is read by a search or a review, and the history exists to answer "what changed about what it looks for" — which a version per rename would bury, since a spec version does not even record the title, so each one would be a copy of the last. Pausing has always worked this way (P1-14); until v1.42 the other three were saved through the spec save and wrote a version only because they shared its path. A change that touches both — a rename alongside a new summary — travels with the version that saves the spec.

**An item becomes active only when its spec can poll** (v1.43, P1-26): at least one criterion, and at least one enabled search plan on a marketplace `settings.sources` switches on. Without a plan nothing is searched; without a criterion the rules of §7 step 6 have nothing to fail, so every listing the pre-filter keeps is a match. The settings all have defaults and reference images are optional, so neither is asked for. `readinessGaps` in the core domain is the one statement of the rule: the item page marks each gap beside its section and holds Start Polling back until there are none, and the store refuses any create, save or patch that would make a not-yet-active item active while one remains. An item already active is not stopped by an edit — the rule guards becoming active, not staying so. This field was a Postgres interval until v1.26; it holds the same ISO 8601 vocabulary `SpecSettings.pollEvery` uses, because the two are one field in the UI and a value that parsed one way in JSONB and another in a column is a bug waiting for whoever writes the second editor.
...
- **Seller content.** Descriptions are reduced to **plain text** at ingest — tags stripped, the contents of `<script>` and `<style>` stripped with them, entities decoded — and rendered as text; never inline in emails. This clause named DOMPurify until v1.33 and allowed either text or sanitised HTML. P1-15 took the text half, because sanitised HTML means DOMPurify, which server-side means jsdom: ten megabytes of dependency in an image §11 budgets at 400 MB slim, to preserve a seller's table layout. Text is safe by construction rather than by trusting a filter — nothing is ever inserted as markup, so there is no bypass to find — and it costs nothing. It also buys something the HTML route would not: eBay returns the description as a full HTML document, and the reviewer's character budget was being spent on `<font face="Arial">` rather than on what the seller wrote (§7 step 1 already called for "text cleaned"; nothing had done it until P1-15).
...
- **Headers.** Every response carries HSTS, a same-origin Content-Security-Policy (the built app has nothing inline; images may be `data:`), `frame-ancestors 'none'`, `nosniff` and a strict referrer policy, set by the API rather than Caddy so they are tested with the app. Database dumps are written owner-only, and container logs are rotated so they cannot fill the disk.
...
Reference and grading example images are not swept by age: they belong to a spec version or a scale and live as long as it does. What the same job does collect, from v1.35, is **orphaned media** — a file a section editor stored whose entry was never saved into a version, because the upload happens on the click and the document that points at it only on save. A file that no spec version, grading scale, candidate or wanted item's display image (v1.42) references, and that is more than a day old, is deleted; the day is so that an editor left open over lunch does not lose its images.
...
*Wanted items* — a grid of cards, each headed by the item's display image or, without one, its category's icon on its tint, with the category named in a small badge over the corner of the picture; then the title, the status and mode as pills, the matched and uncertain counts in their verdict colours, and the last poll, whose date sits behind a clock (as it does on the item page); filterable by category, as the wish list is (cards from v1.42, P1-25, so the page reads as a shelf rather than as the wish list's rows). Item page: the candidate counts in their verdict colours, then sections that each fold away, all but the first starting folded — details (the summary and how sellers list it), the search-plan table with per-query stats, criteria, settings, and labelled reference images (in that order from v1.43: what a new item must fill in comes before the settings, which have defaults) — with a pencil on each part of the spec opening an editor for that section alone, in a modal, whose save is a new version (v1.41, P1-24). Details carries the title, category and status too, a JSON button edits the whole document in a modal, and a clock beside it opens the version history in another; an item is created from a dialog on the list — the Details editor's fields with the status locked to a draft and a summary required — and lands on its own page as version 1, where a red mark beside each section says what it still needs before it can poll (v1.43, P1-26); there is no editor page. The Reference Images section also holds the display image, chosen from the reference images or uploaded, and saved without a version. The version history gains diffs (Phase 3). The candidate list is filtered by day, verdict and origin — opening on today's matches, with each other verdict one filter away and no view of every verdict at once (v1.40). Today is the owner's day, dated as the dashboard's *Today* tiles date it — by the newest verdict, or by when a still-queued candidate was found — so a tile and the list it opens agree; *All* is one filter away, and an item page's all-time counts open it (v1.42). "Amend" opens the chat, and "Scan current listings" runs a backfill.
...
**Phase 3 — Interviewer.** Chat UI with streaming, typed `propose_spec` (settings/criteria split), backfill-before-agree as calibration, labelled image upload, optional `search_images`, spec versioning with side-by-side diffs, Agree flow, amendment flow, "Show prompt" on verdicts. Direct form editing was listed here until v1.35 and is Phase 1 work (P1-18). Whether the spec card the chat shows and the form that edits it become one component is decided here, where the chat is the third renderer and the shape of the answer is known. *Exit: you create the Power Mac 5500 item by conversation and amend it.*
...
**Phase 6 — Open-source release.** README and RUNNING.md, ADAPTERS.md with the `_template` package, `.env.example`, MIT licence, issue templates, a "sources status" table maintained in the repo, and the first-run setup token from §12 so a stranger's instance cannot be claimed by someone else. *Exit: someone else can run their own instance in fifteen minutes.*
